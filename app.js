...
function saveUsername(value) {
  const clean = value.replace(/[^a-zA-Z0-9_.]/g, '').replace(/[_.]{2,}/g, m => m[0]).replace(/^[_.]+/, '').slice(0, 25);
...
function loadUsername() {
  ...
  DOM.charCounter.textContent = `${name.length}/25`;
  if (name) DOM.profileHeader.textContent = `@${name.toLowerCase()}`;
  return name;
}
...
DOM.usernameInput.addEventListener('input', (e) => {
  const val = e.target.value.replace(/[^a-zA-Z0-9_.]/g, '').replace(/[_.]{2,}/g, m => m[0]).replace(/^[_.]+/, '').slice(0, 25);
  e.target.value = val;
  const len = val.length;
  DOM.charCounter.textContent = `${len}/25`;
  if (len >= 20) {
  ...
  if (len > 0) {
    DOM.profileHeader.textContent = `@${val.toLowerCase()}`;
...
async function loadFeed() {
  ...
  const MIN_WARM_POSTS = 15;
  if (cached?.posts?.length >= MIN_WARM_POSTS) {
    ...
    postBuffer = remainder.filter(p => p && !processedIds.has(p.id));
    postBuffer.forEach(p => processedIds.add(p.id));
    DOM.list.innerHTML = '';
    ...
    rotateAndRefillCache(cached.posts);
...
async function renderListItems(items, isPublic = false) {
  ...
  const translatedItems = items.map(item => ({
    ...item,
    _translatedContent: item._translatedContent || item.content || item.text || ''
  }));
  ...
  if (isPublic && currentTab === 'public') {
    const tokenAtRender = loadFeedToken;
    const payload = items.map(item => ({
      firebaseId: item.firebaseId || item.id || null,
      content: item.content || item.text || '',
      authorId: item.authorId || null
    }));

    Translator.translateBatchWithIds(payload)
      .then((translatedTexts) => {
        if (currentTab !== 'public') return;
        if (tokenAtRender !== loadFeedToken) return;
        ...
        translatedTexts.forEach((translated, i) => {
          ...
          if (!postEl) return;
          const body = postEl.querySelector('.post-body');
          if (!body) return;
          body.innerHTML = renderSmartText(translated || items[i].content || items[i].text || '');
        });
      })
      .catch(() => {});
  }
...
function injectSinglePost(item, position = 'top') {
  ...
  if (position === 'top') {
    ...
    setTimeout(() => {
      ...
      watchPostCounts(item.id);
      if (currentTab === 'public' && item.isFirebase) {
        Translator.translateBatchWithIds([{
          firebaseId: item.firebaseId || item.id || null,
          content: item.content || item.text || '',
          authorId: item.authorId || null
        }])
          .then(([translated]) => {
            if (currentTab !== 'public') return;
            const body = postNode.querySelector('.post-body');
            if (!body) return;
            body.innerHTML = renderSmartText(translated || item.content || item.text || '');
          })
          .catch(() => {});
      }
...
function watchPostCounts(postId) {
  window.pendingPostUpdates = (window.pendingPostUpdates || 0) + 1;
  let settled = false;
  const settle = () => {
    if (settled) return;
    settled = true;
    window.pendingPostUpdates = Math.max(0, (window.pendingPostUpdates || 0) - 1);
  };
  ...
  if (type === 'empty') {
    ...
    html = `
      <div id="public-placeholder" class="flex flex-col items-center justify-center w-full text-center px-6 border-2 border-dashed border-slate-100 lg:border-slate-200 rounded-xl mx-auto max-w-[95%]" style="min-height: calc(100vh - 418px);">
        ...
        <button onclick="window.location.reload()" class="mt-5 px-4 py-2 rounded-xl bg-slate-50 border border-slate-100 text-slate-600 text-xs font-semibold hover:bg-slate-100 active:scale-95 transition-all">
          Retry
        </button>
      </div>`;
  }
  ...
  try {
    ...
    if (isPublic) {
      ...
      const docRef = await addDoc(collection(db, "globalPosts"), {
        ...
        serialId: serialId,
        userId: auth.currentUser?.uid,
		originalLang: Translator.getLang(),
        createdAt: serverTimestamp()
      });
    ...
    async () => {
      ...
      try {
        ...
        if (targetIndex !== -1) {
		  ...
		  posts[targetIndex].reportCount = 0;
          localStorage.setItem('freeform_v2', JSON.stringify(posts));
...
function createPostNode(item) {
  ...
  el.className = `feed-item ui-card block w-full bg-white px-4 py-3 mb-3 pb-6 border-b border-slate-100 lg:border-b-[1px] lg:border-slate-200 relative transition-colors ${cursorClass}`;
  ...
  el.innerHTML = `
    ...
    <div class="flex justify-between items-start mb-3">
      <div class="flex items-center gap-2">
        <span class="post-identity px-0 py-0.5 rounded text-[12px] font-bold tracking-wider ${identityClass}">
          ${identityText}
        </span>
        <span class="post-time text-xs text-slate-400 font-medium">${time}</span>
      </div>
  ...
  if (!item.isFirebase || isMyGlobalPost) {
    ...
    delBtn.className = "post-delete-btn absolute top-4 right-4 text-slate-300 hover:text-red-500 transition-colors z-10 p-2";
    delBtn.innerHTML = "✕";
    delBtn.onclick = (e) => {
      e.stopPropagation();
...
// Maps known shortlink / branded domains to their real display name.
// Called by renderSmartText BEFORE the generic subdomain-stripping logic,
// so these always win over whatever the regex would have guessed.
function getSmartBrandName(hostname) {
  ...
  return BRAND_MAP[h] ?? null; // null = no match, fall through to default logic
}
...
function renderSmartText(rawText) {
  ...
  return rawText.replace(urlPattern, (url) => {
    try {
      ...
      let brandName = getSmartBrandName(urlObj.hostname);
      if (brandName === null) {
...
//Flag or Report system inside the comment Modal 
async function handleReport(postId) {
  const reportedKey = `reported_${postId}`;
  console.log('🚩 [handleReport] postId:', postId);
  console.log('🚩 [handleReport] reportedKey:', reportedKey);
  console.log('🚩 [handleReport] already reported:', localStorage.getItem(reportedKey));

  if (!postId) {
    showToast("Cannot report this post", "error");
    return;
  }

  if (localStorage.getItem(reportedKey)) {
    showToast("You've already reported this post", "neutral");
    return;
  }

  showDialog(
    "Report this post?",
    "This will flag the post for review. Thank you for helping keep the community safe.",
    "Report",
    async () => {
      try {
        const { error } = await _supabase.rpc('increment_report_count', { post_id: postId });
        if (error) throw error;
        localStorage.setItem(reportedKey, '1');
        console.log('🚩 [handleReport] saved to localStorage:', reportedKey);
        const btn = document.getElementById('reportPostBtn');
        if (btn) {
          btn.classList.add('text-red-400', 'bg-red-50');
          btn.disabled = true;
        }
        showToast("Post reported. Thank you.", "neutral");
      } catch (err) {
...
if (reportBtn && realFirestoreId) {
	console.log('🚩 [openModal] wiring report button to postId:', realFirestoreId);
  // Reset state first — clears leftover style from previous modal open
  reportBtn.style.display = '';
  reportBtn.classList.remove('text-red-400', 'bg-red-50');
  reportBtn.disabled = false;

  // Grey it out if already reported
  if (localStorage.getItem(`reported_${realFirestoreId}`)) {
    reportBtn.classList.add('text-red-400', 'bg-red-50');
    reportBtn.disabled = true;
  }
  reportBtn.onclick = () => handleReport(realFirestoreId);
} else if (reportBtn) {
  // No firebase ID — hide the button entirely (private post)
...
// ─── Keyboard modal: open / close ────────────────────────────────────────────

const sheet = document.getElementById('keyboard_adjuster');

// Correct formula: the visible height IS the viewport height.
// We do NOT subtract from window.innerHeight — that introduces double-counting.
function _applyKeyboardHeight() {
  const vv = window.visualViewport;
  // clamp: never go below 200px (malformed resize event) or above full screen height
  const h = Math.max(200, Math.min(window.innerHeight, vv.height + vv.offsetTop));
  sheet.style.height = h + 'px';
}

// rAF debounce — fires at most once per frame, killing resize jitter
let _rafId = null;
...
// Feature-detect whether Layer 1+2 already handled it.
// If dvh shrank when the keyboard opened, the sheet height from CSS will be
// less than window.innerHeight — meaning the browser handled it. Skip JS.
function _needsJsFallback() {
  const vv = window.visualViewport;
  // If the viewport is at full height, keyboard isn't open yet — JS is fine to attach.
  // The real check happens on the first resize event inside the handler.
  return !!vv; // only skip on browsers with no visualViewport at all
}
...
function openInputModal() {
  ...
  // Reset to CSS-driven height first — Layer 1 & 2 may handle it entirely
  sheet.style.height = '';

  setTimeout(() => DOM.input.focus(), 150);

  // Attach Layer 3 only if visualViewport exists (universal, but gracefully absent in some webviews)
  if (window.visualViewport) {
    window.visualViewport.addEventListener('resize', _onViewportResize);
    window.visualViewport.addEventListener('scroll', _onViewportResize); // iOS sometimes fires scroll instead
  }
}
...
function closeInputModal() {
  ...
  // Cancel any pending rAF to avoid a stale resize applying after close
  if (_rafId) {
  ...
  // Reset to CSS height so it's clean for next open
  sheet.style.height = '';
}
...
function getSmartShareButtons(text) {
  ...
  limit: 10000, 
  name: 'Reddit',
  icon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M6.167 8a.833.833 0 1 0 .833.833A.833.833 0 0 0 6.167 8m4.5 0a.833.833 0 1 0 .833.833.833.833 0 0 0-.833-.833M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m3.498-4.048a1.207 1.207 0 0 0-1.768-.394c-1.08.46-2.448.747-3.941.747-.51 0-1.011-.034-1.493-.1a2.495 2.495 0 0 0-4.447-1.066 1.2 1.2 0 0 0-1.482.795c-.437.069-.876.19-1.308.363-.802.318-1.59.69-2.374 1.12a2.73 2.73 0 0 0-2.612.35 1.207 1.207 0 0 0-.314 1.819c-.232.04-.445.129-.626.273-.38.306-.58.771-.58 1.213 0 .122.01.239.032.342-.001.031-.002.063-.002.095 0 2.366 3.109 4.293 6.93 4.293s6.93-1.927 6.93-4.293c0-.032-.001-.064-.002-.095.021-.103.032-.22.032-.342 0-.442-.2-.907-.58-1.213a1.491 1.491 0 0 0-.626-.273 1.207 1.207 0 0 0-.314-1.819 2.73 2.73 0 0 0-2.612-.35c-.784-.43-1.572-.802-2.374-1.12-.432-.173-.871-.294-1.308-.363a1.2 1.2 0 0 0-1.482-.795 2.495 2.495 0 0 0-4.447 1.066 2.428 2.428 0 0 0-1.493.1c-1.493 0-2.861-.287-3.941-.747a1.207 1.207 0 0 0-1.768.394Z"/></svg>',
  classes: 'bg-orange-50 text-orange-600 border-orange-200 hover:bg-orange-600 hover:border-orange-600 hover:text-white'
    }
  ];
...
// Cleans up index.html/app.html from the URL so shared links look tidy.
...
async function sharePost(text, platform) {
  ...
  switch (platform) {
    ...
    case 'telegram':
      ...
      break;
    case 'reddit':
      url = `https://www.reddit.com/submit?url=${urlLink}&title=${urlText}`;
      break;
  }
...
window.saveToLocal = function(roomId, msgObj) {
    ...
    const lang = (typeof Translator !== 'undefined' && Translator.getLang) ? (Translator.getLang() || 'EN') : 'EN';
    const isIncoming = msgObj?.senderId && msgObj.senderId !== MY_USER_ID;
    const alreadyTranslated = !!msgObj?.translations?.[lang];
    if (isIncoming && msgObj?.text?.trim() && !alreadyTranslated) {
        const id = msgObj.id;
        const rawText = msgObj.text;
        Translator.translateBatch([rawText])
            .then(([translated]) => {
                if (!translated || translated === rawText) return;
                let h = [];
                try { h = JSON.parse(localStorage.getItem(key) || '[]'); } catch { h = []; }
                const idx = h.findIndex(m => m.id === id);
                if (idx === -1) return;
                h[idx].translations = h[idx].translations || {};
                if (h[idx].translations[lang]) return;
                h[idx].translations[lang] = translated;
                localStorage.setItem(key, JSON.stringify(h));
                if (window.currentChatRoomId === roomId) {
                    const el = document.querySelector(`[data-dm-msg-id="${id}"]`);
                    if (el) el.innerHTML = renderSmartText(translated);
                }
            })
            .catch(() => {});
    }
...
window.renderMessages = function(roomId) {
 ...
 //   console.log("📦 Loaded History Count:", history.length);
	...
	// Process the text to catch links and turn them into pretty pills
        const displayText = (msg.translations && msg.translations[lang]) ? msg.translations[lang] : msg.text;
        const processedText = renderSmartText(displayText);
    return `
            ...
            <div class="max-w-[85%] md:max-w-[75%] rounded-2xl px-4 py-2 ${
                ...
                <div class="flow-root">
                    <p class="text-sm inline break-words leading-normal">
                        <span data-dm-msg-id="${msg.id}">${processedText}</span>&nbsp;
    ...
    const needs = history.filter(m => m?.senderId && m.senderId !== MY_USER_ID && m?.text?.trim() && !(m.translations && m.translations[lang]));
    if (needs.length > 0 && typeof Translator !== 'undefined' && Translator.translateBatch) {
        const tokenRoom = roomId;
        const texts = needs.map(m => m.text);
        Translator.translateBatch(texts)
            .then((translatedTexts) => {
                if (window.currentChatRoomId !== tokenRoom) return;
                let h = [];
                try { h = JSON.parse(localStorage.getItem(tokenRoom) || '[]'); } catch { h = []; }
                const byId = new Map(h.map(m => [m.id, m]));
                let changed = false;
                needs.forEach((msg, i) => {
                    const translated = translatedTexts[i] || msg.text;
                    if (!translated || translated === msg.text) return;
                    const stored = byId.get(msg.id);
                    if (!stored) return;
                    stored.translations = stored.translations || {};
                    if (stored.translations[lang]) return;
                    stored.translations[lang] = translated;
                    changed = true;
                    const el = document.querySelector(`[data-dm-msg-id="${msg.id}"]`);
                    if (el) el.innerHTML = renderSmartText(translated);
                });
                if (changed) localStorage.setItem(tokenRoom, JSON.stringify(Array.from(byId.values())));
            })
            .catch(() => {});
    }
};
...
function showDialog(title, message, confirmText, onConfirm) {
  ...
  const isDestructive = confirmText && (
  confirmText.toLowerCase().includes('delete') ||
  confirmText.toLowerCase().includes('report')
);
...
//SPAM GUARD 
//Levensthein distance 85% 
function levenshteinSimilarity(a, b) {
  const s1 = a.toLowerCase().trim();
  const s2 = b.toLowerCase().trim();
  if (s1 === s2) return 1;
  if (!s1.length || !s2.length) return 0;
  const matrix = Array.from({ length: s2.length + 1 }, (_, i) =>
...
function checkSpamGuard(newContent) {
  ...
  if (levenshteinSimilarity(newContent, history.lastContent) >= 0.85) {
    history.repeatCount++; 
  } else {
...
// ─── TRANSLATION MODULE ───────────────────────────────────────────────
// run Translator.getApiCallCount() in console
var Translator = (function() {
  ...
  if (storedLang) {
    ...
    const language = localStorage.getItem('freeform_language') || 'en';
  ...
  // Original batch — still used for private posts (no firebaseId)
  ...
  /**
   * Translates a batch of public posts.
   * For each post:
   *   1. Check in-memory cache
   *   2. Check Firebase (translations.{LANG} field on the post doc)
   *   3. Fall back to translation API
   *   4. Write new translations back to Firebase (fire-and-forget)
   *
   * @param {Array<{ firebaseId: string|null, content: string }>} items
   * @returns {Promise<string[]>} translated strings, same order as input
   */
async function translateBatchWithIds(items) {
  console.log(`\n🌐 [Translator] ═══════ translateBatchWithIds START ═══════`);
  console.log(`🌐 [Translator] currentLang: ${currentLang}, _db ready: ${!!_db}`);
  console.log(`🌐 [Translator] items received:`, items.length, items.map(i => ({ id: i.firebaseId, preview: (i.content||'').slice(0,30) })));

  const lang = currentLang;
  const results = new Array(items.length);

  // --- Step 1: Memory cache pass ---
  console.log(`\n📦 [Translator] ── STEP 1: Memory cache check ──`);
  const needsFirebase = [];
  items.forEach((item, idx) => {
  const text = item.content || '';
  const cacheKey = `${item.firebaseId}:${lang}`;
  if (!text.trim()) {
    console.log(`  [${idx}] ⚠️  Empty text, skipping — id:${item.firebaseId}`);
    results[idx] = text;
  } else if (item.authorId && item.authorId === MY_USER_ID) {
    console.log(`  [${idx}] 👤 Own post — skipping translation`);
    results[idx] = text;
  } else if (cache[cacheKey]) {
    console.log(`  [${idx}] ✅ Memory cache HIT — key:${cacheKey}`);
    results[idx] = cache[cacheKey];
  } else {
    console.log(`  [${idx}] ❌ Memory cache MISS — key:${cacheKey}, queuing for Firebase`);
    needsFirebase.push({ index: idx, item });
  }
});
  console.log(`📦 [Translator] Memory: ${items.length - needsFirebase.length} hits, ${needsFirebase.length} misses`);

  if (needsFirebase.length === 0) {
    console.log(`✅ [Translator] All served from memory cache — done\n`);
    return results;
  }

  // --- Step 2: Firebase cache pass ---
  console.log(`\n🔥 [Translator] ── STEP 2: Firebase cache check ──`);
  if (!_db) {
    console.error(`🔥 [Translator] _db is NULL — Translator.init(db) was never called or db wasn't passed!`);
  }

  const firebaseFetches = needsFirebase.map(async ({ index, item }) => {
    if (!item.firebaseId) {
      console.log(`  [${index}] ⚠️  No firebaseId — skipping Firebase, will go straight to API`);
      return { index, item, firebaseHit: null };
    }
    console.log(`  [${index}] 🔍 Fetching from Firebase — doc:globalPosts/${item.firebaseId}`);
    try {
      const snap = await getDoc(doc(_db, 'globalPosts', item.firebaseId));
      if (!snap.exists()) {
        console.log(`  [${index}] ❌ Firebase doc does not exist — id:${item.firebaseId}`);
        return { index, item, firebaseHit: null };
      }
      const allTranslations = snap.data()?.translations || {};
      console.log(`  [${index}] 📄 Firebase doc exists — stored translations:`, Object.keys(allTranslations));
      const stored = allTranslations[lang] ?? null;
      if (stored) {
        console.log(`  [${index}] ✅ Firebase HIT for lang:${lang} — "${stored.slice(0, 40)}..."`);
      } else {
        console.log(`  [${index}] ❌ Firebase MISS for lang:${lang} — translation not stored yet`);
      }
      return { index, item, firebaseHit: stored };
    } catch (err) {
      console.warn(`  [${index}] 💥 Firebase fetch ERROR — id:${item.firebaseId}`, err);
      return { index, item, firebaseHit: null };
    }
  });

  const firebaseResults = await Promise.all(firebaseFetches);

  const needsApi = [];
  firebaseResults.forEach(({ index, item, firebaseHit }) => {
    if (firebaseHit) {
      results[index] = firebaseHit;
      cache[`${item.firebaseId}:${lang}`] = firebaseHit;
    } else {
      needsApi.push({ index, item });
    }
  });
  console.log(`🔥 [Translator] Firebase: ${firebaseResults.length - needsApi.length} hits, ${needsApi.length} misses`);

  if (needsApi.length === 0) {
    console.log(`✅ [Translator] All remaining served from Firebase — done\n`);
    return results;
  }

// --- Step 3: Translation API ---
console.log(`\n🤖 [Translator] ── STEP 3: Translation API call ──`);
const textsToTranslate = needsApi.map(({ item }) => item.content || '');
const requestPayload = { texts: textsToTranslate, targetLang: lang };
apiCallCount++; 
console.log(`🤖 [Translator] API call %c#${apiCallCount}%c — sending ${textsToTranslate.length} texts, targetLang:${lang}`, 'color: red; font-weight: bold; font-size: 16px;', 'color: inherit;');
console.log(`🤖 [Translator] Full request payload:`, JSON.stringify(requestPayload));
let apiTranslations = textsToTranslate;
try {
  const res = await fetch(WORKER_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  console.log(`🤖 [Translator] API response status:`, res.status);
  const data = await res.json();
  console.log(`🤖 [Translator] API raw response:`, JSON.stringify(data));
  console.log(`🤖 [Translator] Expected ${textsToTranslate.length} translations, got:`, data.translations?.length);
  apiTranslations = textsToTranslate.map((t, i) => {
    const result = data.translations?.[i] ?? t;
    console.log(`🤖 [Translator] [${i}] "${t.slice(0,30)}" → "${result.slice(0,30)}" (changed: ${t !== result})`);
    return result;
  });
} catch (err) {
  ...
  console.log(`\n💾 [Translator] ── STEP 4: Persisting to Firebase ──`);
  needsApi.forEach(({ index, item }, i) => {
    const translated = apiTranslations[i];
    results[index] = translated;
    cache[`${item.firebaseId}:${lang}`] = translated;

    if (item.firebaseId) {
      console.log(`  💾 Writing to Firebase — post:${item.firebaseId} lang:${lang} text:"${translated.slice(0, 40)}..."`);
      updateDoc(doc(_db, 'globalPosts', item.firebaseId), {
        [`translations.${lang}`]: translated
      }).then(() => {
        console.log(`  ✅ Firebase write SUCCESS — post:${item.firebaseId} lang:${lang}`);
      }).catch(err => {
        console.warn(`  ❌ Firebase write FAILED — post:${item.firebaseId}`, err);
      });
    } else {
      console.log(`  ⚠️  Skipping Firebase write — no firebaseId for index:${index}`);
    }
  });

  console.log(`\n✅ [Translator] ═══════ translateBatchWithIds END ═══════\n`);
  return results;
}
  ...
  return { init, translateText, translateBatch, translateBatchWithIds, setLang, getLang, getApiCallCount: () => apiCallCount };
})();
console.log('%c🌐 [Translator] exposed methods: ' + Object.keys(Translator).join(', '), 'background: orange; color: black; font-size: 14px; padding: 4px 8px; border-radius: 4px;');
...
async function resetAppCompletely() {
  ...
  showDialog(
    ...
    "Delete", // Still contains 'Delete' to trigger your red styling
    async () => {
      try {
