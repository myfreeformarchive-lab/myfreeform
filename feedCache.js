  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = (e) => {
      ...
      const delTimeout = setTimeout(() => {
        finish(reject, new Error('IndexedDB delete timed out'));
      }, DELETE_TIMEOUT_MS);

      const del = indexedDB.deleteDatabase(CACHE_DB_NAME);
      del.onblocked = () => {
        clearTimeout(delTimeout);
        finish(reject, new Error('IndexedDB delete blocked'));
      };
      del.onerror = () => {
        clearTimeout(delTimeout);
        finish(reject, del.error || new Error('IndexedDB delete failed'));
      };
      del.onsuccess = () => {
        clearTimeout(delTimeout);
        const req2 = indexedDB.open(CACHE_DB_NAME, CACHE_VERSION);
        req2.onupgradeneeded = (e2) => {
          const db2 = e2.target.result;
          if (!db2.objectStoreNames.contains(CACHE_STORE)) db2.createObjectStore(CACHE_STORE);
          console.log('📦 Store recreated after self-heal ✅');
        };
        req2.onsuccess = (e2) => finish(resolve, e2.target.result);
        req2.onerror = (e2) => finish(reject, e2.target.error);
      };
    };
...
export async function readCache() {
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      tx.oncomplete = () => { try { idb.close(); } catch {} };
...
export async function writeCache(data) {
  ...
  if (!data?.posts || !Array.isArray(data.posts) || data.posts.length === 0) return false;
  const serialized = {
    ...data,
    v: CACHE_V,
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      tx.oncomplete = () => { console.log('💾 writeCache: tx complete ✅'); try { idb.close(); } catch {} resolve(true); };
...
export async function clearCache() {
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      tx.oncomplete = () => { console.log('🗑️ clearCache: done ✅'); try { idb.close(); } catch {} resolve(true); };
